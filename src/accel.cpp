...
bool AABB::intersect(const Ray &ray, Float *t_in, Float *t_out) const {
  ...
  const Vec3f &inverse_dir = ray.safe_inverse_direction;
  const Vec3f &t_min = Min( (low_bnd - ray.origin) * inverse_dir, (upper_bnd - ray.origin) * inverse_dir);
  const Vec3f &t_max = Max( (low_bnd - ray.origin) * inverse_dir, (upper_bnd - ray.origin) * inverse_dir);;
